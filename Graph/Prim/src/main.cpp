...
int main(int argc, char **argv){
    ...
    cout << nodes.front()->getValue() << endl;
    Grp->Prim(nodes.front());
    cout << "Nodo sorgente " << " Nodo Destinazione " << " Dis NS " << " Dis ND" << endl;
    list<Node*>::iterator nodeIter;
    for(nodeIter = nodes.begin(); nodeIter != nodes.end(); ++nodeIter){
        if((*nodeIter)->getParent() == nullptr)
            cout << "NULL" << " " << (*nodeIter)->getValue() << " " 
            << (*nodeIter)->getDistance() << " " << "0" << endl;
        else if(((*nodeIter)->getParent() == nullptr) && (*nodeIter)->getDistance() != numeric_limits<int>::max())
            cout << "NULL" << " " << (*nodeIter)->getValue() << " " << "N.R." << (*nodeIter)->getParent()->getDistance() << " " << endl;
        else if((*nodeIter)->getDistance() == numeric_limits<int>::max())
            cout << (*nodeIter)->getParent()->getValue() << " " << (*nodeIter)->getValue() << " " << "N.R." << (*nodeIter)->getParent()->getDistance() << endl;
        else
            cout << (*nodeIter)->getParent()->getValue() << " " << (*nodeIter)->getValue() << " " 
            << (*nodeIter)->getDistance() << " " << (*nodeIter)->getParent()->getDistance() << endl;
    }
